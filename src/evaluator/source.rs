...
//!
//! Reads source files and verifies they contain (or don't contain) expected
//! patterns.

use std::fs;

use crate::{error, evaluator::loader::SourceAssertion};
...
pub fn check_source_assertion(assertion: &SourceAssertion) -> error::Result<()> {
    let content =
        fs::read_to_string(&assertion.file).map_err(|e| error::RaraBddError::SourceAssertion {
    ...
    if let Some(regex_str) = &assertion.matches {
        let re =
            regex::Regex::new(regex_str).map_err(|e| error::RaraBddError::SourceAssertion {
